 ...
 * 
 * Unlike CustomerAuthMiddleware, this does NOT require authentication.
 * If a customer session exists, it attaches the user to the request.
 * If no session exists, it allows the request to continue as a guest.
 */
class OptionalCustomerAuthMiddleware implements Middleware
{
    public function handle(Request $request, callable $next): Response
    {
        ...
        if ($customerId) {
            ...
            // Customer no longer exists or disabled - clear session
            $session->remove('customer_id');
            $session->remove('customer_username');
            $session->remove('customer_email');
        }

        // No customer session - continue as guest
        return $next($request);
    }
